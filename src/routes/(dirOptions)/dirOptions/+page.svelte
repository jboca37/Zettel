...
<script lang="ts">
  ...
  async function clearStore() {
    ...
    const directoriesStore = await load("directories.json", {
      autoSave: false,
    });
    await directoriesStore.reset();
    console.log("Final store state:", await directoriesStore.entries());

    const userDataStore = await load("user.json", { autoSave: false });
    await userDataStore.reset();
    console.log("Final store state:", await userDataStore.entries());

    const awardsStore = await load("achievements.json", { autoSave: false });
    await awardsStore.reset();
    console.log("Final store state:", await awardsStore.entries());
  }
</script>
  ...
  <div class="flex items-center w-full flex-col">
    <div class="flex items-center w-full flex-row">
      <div class="flex items-start w-full flex-col mx-6">
        ...
        <p class="text-neutral-400">Create a New Zettel Vault Under a Folder</p>
      </div>
      <a
